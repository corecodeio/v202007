...
import { FooterText, FooterView, PrimaryText, SecondaryText, TertiaryText, View } from '../../../common/component';
...
export const SendPhoneNumberVerificationCode: React.FC<Props> = ({ navigation }) => {
	const dependencies = React.useContext(DependencyContext);
	const onboarding = dependencies.provide(OnboardingInjectionKey);

	const [args, setInput] = React.useState<QuerySendPhoneNumberVerificationCodeArgs>({
		input: {
			phoneNumber: '',
		},
	});
	...
	return (
		<View container flex={1} justifyContent="center">
			...
			<SecondaryText paddingTop={30}>Selecciona tu código de país e ingresa tu numero de teléfono</SecondaryText>
			{/* <TextInput onChange={(e) => onSetPhoneNumber(e.target.valueOf())} /> */}
			<IntlTelInput onSetPhoneNumber={onSetPhoneNumber} />
			{sendPhoneNumberVerificationCodeError && <Text>Error al enviar el SMS. Intenta de nuevo.</Text>}
			<PrimaryButton mb={4} onPress={onSendPhoneNumberVerificationCode} disabled={!args.input.phoneNumber}>
				Continuar
			</PrimaryButton>
			<TertiaryText>Al crear tu cuenta aceptas los Términos y Condiciones</TertiaryText>
